...
export default function UserAccount(props: AccountItemProps) {
  const { allAccounts, userObj, isHomeRefresh, handleP2PUpdated } = props;
  ...
  const handleAuth = () => {
    if (userObj !== null && userName && password) {
      axios
        .post(
          `${process.env.REACT_APP_SERVER}/api/${editingCompany?.nickname}/is_valid`,
          {
            id: userName,
            pwd: password,
          },
  ...
  const handleSubmit = () => {
    ...
    if (userObj !== null && isAuthentic) {
      axios
        .post(
          `${process.env.REACT_APP_SERVER}/api/register/company_update`,
          data,
          {
            headers: {
              Authorization: "Token " + userObj.auth_token,
  ...
  return (
    <div>
      <Autocomplete
        ...
        options={(value === null ? allAccounts : filteredCompany).map(
