...
export default function Profile(props: ProfileProps) {
  const { handleLogOut, getUserDataOfCompany, myPoint, updatePoint, userObj } =
    props;
  ...
  const handleUploadImage = () => {
    if (userObj !== null) {
      axios
        .post(
          `${process.env.REACT_APP_SERVER}/api/auth/image_upload`,
          imgUpload,
          {
            headers: {
  ...
  const getProfileImg = () => {
    if (userObj !== null) {
      axios
        .post(`${process.env.REACT_APP_SERVER}/api/auth/get_image`, null, {
          headers: {
            Authorization: "Token " + userObj.auth_token,
  ...
  return (
    <div className="profile">
      <Card className={classes.card} elevation={3}>
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <CardHeader
            title={
              ...
              isMobile &&
              props.userObj.username.length > 3
                ? props.userObj?.username.substring(0, 3) + "..."
                : props.userObj?.username) +
              ` (${userObj?.email})` +
              " 님의"
            }
          ...
          ></CardHeader>
          <Dialog open={open}>
            <DialogTitle>프로필 사진 수정하기</DialogTitle>
            <DialogContent>
              <input
          ...
          <CardMedia className={classes.cardMedia}>
            ...
            <IconButton onClick={handleClickRefresh}>
              <UpdateIcon
                className={
                  isMobile ? classes.controlButtonMobile : classes.controlButton
                }
              />
            </IconButton>
            {/* 로그아웃 */}{" "}
        ...
        <CardActions>
          <div style={{ zIndex: 2 }}>
            <Button onClick={handleClickFunding} className={classes.button}>
              나의투자
            </Button>
            <Button onClick={handleClickPoint} className={classes.button}>
              포인트내역
            </Button>
            <Button onClick={handleClickShare} className={classes.button}>
              공유하기
            </Button>
          </div>
          <div>
