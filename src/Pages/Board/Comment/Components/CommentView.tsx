...
function CommentView(props: viewProps) {
  ...
  const handleLikeDisLike = (event: React.MouseEvent, commentId: number) => {
    ...
    if (userObj !== null && item) {
      if (item.like_dislike === -1) {
        ...
        // 공감, 비공감 취소 : 0 or 1 = 공감, 비공감 이미 했을 경우
  ...
  const getDetailComment = (commentId: number) => {
    if (userObj !== null) {
      axios
        .post(
          `${process.env.REACT_APP_SERVER}/api/notice/detail_comment`,
          {
            comment_id: commentId,
          },
          {
      ...
      {item && (
        <>
          <div className={classes.commentContainer}>
            ...
            {/* 내용 */}{" "}
            <li style={{ margin: "20px 0" }}>{item.comment_content}</li>
            {/* 시간 */}{" "}
            <li
              style={{
                display: "flex",
            ...
            >
              ...
              <Typography component="span" className={classes.handButton}>
                <IconButton
                  aria-label="like"
                  className={
                    pressableLike ? classes.button : classes.disabledButton
                  }
                  onClick={(e: React.MouseEvent) =>
                    handleLikeDisLike(e, item.comment_id)
                  }
                >
