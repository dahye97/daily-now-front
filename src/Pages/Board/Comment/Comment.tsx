...
function Comment(props: CommentProps) {
  ...
  /**
   * 답글 실시간 업데이트 처리 함수
   * @param parentId 현재 recomment의 parentId
   */
  ...
  /**
   * 댓글 수정 함수
   * @param commentId 수정하려는 commentId
   */
  ...
  /**
   * 댓글 삭제 함수
   * @param commentId 삭제하려는 commentId
   */
  const handleDelete = (commentId: number, parentId?: number) => {
    if (userObj !== null) {
      axios
        .post(
          `${process.env.REACT_APP_SERVER}/api/notice/delete_comment`,
          {
  ...
  return (
    <>
      {/* 댓글 */}
      <Paper
        className={isMobile ? classes.commentPaperMobile : classes.commentPaper}
      >
        <p>
          댓글{" "}
          <b style={{ color: "red" }}>
                    ...
                    <AccordionDetails className={classes.recommentContainer}>
                      <div>
                        {recommentList.map((recommentItem) => {
                          return isEditing ===
                            "panel" + recommentItem.comment_id ? (
                            <>
                              {/* 📌 답글 수정 창 */}
                              <CommentForm
