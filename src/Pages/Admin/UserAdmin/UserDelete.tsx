...
export default function UserDelete(props:UserDeleteProps) {
     ...
     const handleSubmit= () => {
          ...
          axios.post(`${process.env.REACT_APP_SERVER}/api/admin/user/user_delete`, {
              email: emailList
          },{
               headers: {
     ...
     return (
          <div>
               ...
               {selectedUser.map( user => {
                    return (
                         <List>
                              <ListItem>
							<ListItemText 
                                   primary={ <div>{user.id} {user.username}</div> } 
               ...
               <Button color="primary" variant="contained" onClick={handleSubmit}>회원 삭제</Button>    
