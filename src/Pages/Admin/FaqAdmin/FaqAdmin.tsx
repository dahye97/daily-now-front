...
export default function FaqAdmin(props: FaqAdminProps) {
  ...
  const getFAQData = () => {
    axios
      .post(`${process.env.REACT_APP_SERVER}/api/notice/faq_list`, {
        page_size: 10,
      })
      .then((res) => {
  ...
  const handleAddFaq = () => {
    axios
      .post(
        `${process.env.REACT_APP_SERVER}/api/admin/faq/add_faq`,
        {
          question: question,
          answer: answer,
        },
  ...
  const handleDeleteFaq = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_SERVER}/api/admin/faq/delete_faq`,
        {
          faq_id: faqlist,
        },
        {
      ...
      {tabName === "EDIT_FAQ" ? (
        ...
        <>
          ...
          >
            <h2>FAQ 목록 조회</h2>
            <div>
              <Button color="primary" variant="contained" onClick={onOpenEdit}>
                새 FAQ 추가
              </Button>
              <Button
                color="primary"
