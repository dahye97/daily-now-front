...
export default function Navigation(props: NavProps) {
  ...
  const { isAdmin, handleLogOut } = props;
  const theme = useTheme();

  /**
   * 메인 화면 상단 메뉴 리스트
   * id | 0 : 로그인한 유저, 1 : 비로그인 유저
   */
  const menuList = [
  ...
  const myPageMenu = [
    ...
    { access: "admin", id: "1", name: "관리자", url: "/admin" },
  ];
          ...
          <Toolbar className={classes.toolbar}>
            ...
            >
              {myPageMenu.map((myMenu, index) => {
                let menuItem = (
                  <MenuItem key={index} onClick={() => handleClick(myMenu.url)}>
                    {myMenu.name}
                  </MenuItem>
                );
                if (myMenu.access === "admin") {
                  // 관리자 권한 메뉴
                  if (isAdmin) {
                    return menuItem;
                  }
